...
import { generateFirstPeriodicMaintenanceTask } from './periodicMaintenanceTasks.js';
...
export async function applyDeviceActivationResult(
  ...
  try {
    ...
    if (shape.decision === 'activated_successfully') {
      ...
      periodicGeneration = await generateFirstPeriodicMaintenanceTask(
        db,
        Number(vt.device_id),
        performedByUserId,
      );
...
// ════════════════════════════════════════════════════════════════
// Records سحب الجهاز toward the task's service branch. Maintenance retrieval
// moves the device to in_workshop. Replacement retrieval marks the old device
// retrieved, transfers possession to the workshop/service branch, and cancels
// the device's other open tasks.
// ════════════════════════════════════════════════════════════════
