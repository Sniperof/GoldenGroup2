...
export default function TeamAgendaPanel({ appointments, date }: TeamAgendaPanelProps) {
    ...
    return (
        <div className="w-full bg-slate-50 border-slate-200 flex flex-col shrink-0 h-full overflow-hidden shadow-sm z-20">
            {/* Header */}
            <div className="px-4 py-3 border-b border-slate-200 bg-white shrink-0">
                ...
                <p className="text-xs text-slate-500 mt-1 mr-5">جدول زيارات الغد: {date}</p>
            </div>
                    ...
                    return (
                        <div key={slot} className="relative group">
                            ...
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className={`text-xs font-bold ${isBooked ? 'text-emerald-700' : 'text-slate-500'}`}>{slot}</span>
                            </div>
                            ...
                            {isBooked ? (
                                ...
                                <div className="pl-2 border-r-2 border-dashed border-slate-200">
                                    <div className="bg-slate-50/50 border border-dashed border-slate-200 rounded-lg p-2.5 flex items-center justify-center min-h-[50px] group-hover:bg-slate-100/50 transition-colors">
                                        <p className="text-xs text-slate-400">وقت متاح</p>
                                    </div>
                                </div>
                            )}
                        </div>
