...
import { UserPlus, Search, Building2, MapPin, AlertCircle, ArrowRight, XCircle, FilePlus2, Download, Upload, Info, LayoutGrid, List, ShieldCheck, Edit, User } from '../../components/ui/icons';
                                    ...
                                    paginatedCandidates.map((c, idx) => {
                                        ...
                                        return (
                                            <tr key={c.id} className={`${idx % 2 === 1 ? 'bg-slate-50/50' : 'bg-white'} hover:bg-sky-50 transition-colors h-12 group`}>
                                                ...
                                                <td className="px-5 py-2 text-xs">
                                                    <span className={`px-2 py-0.5 rounded text-xs font-black border ${c.status === 'Suggested' ? 'bg-sky-50 text-sky-700 border-sky-100' : c.status === 'Qualified' ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-red-50 text-red-700 border-red-100'}`}>
