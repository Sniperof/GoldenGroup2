    ...
    return (
        <div className="h-full flex flex-col overflow-hidden">
            ...
            <div className="bg-white border-b border-slate-200 px-6 py-4 shrink-0">
                <div className="flex items-center gap-4 mb-3">
                    <button onClick={() => navigate('/planning/overview')}
                        className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center hover:bg-slate-50 transition-colors text-slate-500 hover:text-slate-700 shrink-0">
                    ...
                    <div className="flex-1">
                        <h1 className="text-2xl font-bold text-slate-800">مهام {teamInfo.name}</h1>
                    ...
                    <div className="flex items-center gap-2">
                        <div className="flex flex-col items-center bg-slate-50 rounded-xl border border-slate-200 px-4 py-2 min-w-[70px]">
                            <span className="text-lg font-black text-slate-800">{stats.total}</span>
                            <span className="text-xs text-slate-400 font-medium">إجمالي</span>
                        </div>
                        <div className="flex flex-col items-center bg-emerald-50 rounded-xl border border-emerald-200 px-4 py-2 min-w-[70px]">
                            ...
                            <span className="text-xs text-emerald-500 font-medium">محجوز</span>
                        </div>
                        <div className="flex flex-col items-center bg-amber-50 rounded-xl border border-amber-200 px-4 py-2 min-w-[70px]">
                            ...
                            <span className="text-xs text-amber-500 font-medium">قيد الانتظار</span>
                        </div>
                    </div>
                </div>
                        ...
                        return (
                            <button key={tab.key} type="button" onClick={() => setActiveFilter(tab.key)}
                                className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-bold transition-all ${isActive
                                    ...
                                    : 'bg-white text-slate-500 border border-transparent hover:bg-slate-50 hover:text-slate-700'}`}>
                                <tab.icon className="w-3.5 h-3.5" />
                                <span>{tab.label}</span>
                                <span className={`px-1.5 py-0.5 rounded text-xs font-bold ${isActive ? 'bg-indigo-100 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>{tab.count}</span>
                            </button>
                        );
                    })}
                </div>
                ...
                <table className="w-full">
                    <thead className="bg-slate-50 sticky top-0 z-10">
                        <tr className="border-b border-slate-200">
                            <th className="text-right px-5 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">الزبون</th>
                            <th className="text-right px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">نوع المهمة</th>
                            <th className="text-right px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">المصدر</th>
                            <th className="text-center px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">وقت الزيارة</th>
                            <th className="text-right px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">حالة الاتصال</th>
                            <th className="text-center px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wide">إجراءات</th>
                        </tr>
                    </thead>
                    <tbody>
                        <AnimatePresence>
                            {filteredTasks.map((task, idx) => {
                                ...
                                return (
                                    <motion.tr
                                        ...
                                        className="border-b border-slate-100 hover:bg-sky-50/30 transition-colors group"
                                    >
                                        {/* Customer */}
                                        <td className="px-5 py-3.5">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <a href={`tel:${task.mobile}`} className="text-xs text-sky-600 hover:text-sky-800 flex items-center gap-1 mt-0.5">
                                        ...
                                        <td className="px-4 py-3.5">
                                            <div className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg ${tt.bg} ${tt.color}`}>
                                                ...
                                                <span className="text-xs font-bold">{tt.label}</span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3.5">
                                            {task.source === 'route' ? (
                                                <div className="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-indigo-50 border border-indigo-200 text-indigo-700">
                                                    ...
                                                    <span className="text-xs font-bold">{task.sourceLabel}</span>
                                                </div>
                                            ) : (
            ...
            <div className="bg-white border-t border-slate-200 px-6 py-3 flex items-center justify-between shrink-0">
                ...
                <div className="flex items-center gap-2">
                    {/* Progress bar */}
                    <span className="text-xs text-slate-400 font-medium">نسبة الجدولة</span>
