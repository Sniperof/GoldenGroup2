...
const TASK_GROUP_CONFIG: Record<string, { taskTypes: string[]; emptyLabel: string }> = {
  'device-demo': {
  ...
  // periodic_maintenance is included in the list per the constitution but it
  // is generated by a cron (V2 scope), so it just appears here when present.
  'maintenance': {
 ...
 *         description: Internal Server Error
 */
async function listTaskGroupRows(req: any, res: any, taskTypes: string[], emptyLabel: string) {
  try {
    const authContext = getAuthContext(req);
...
router.get('/device-demo', requirePermission('open_tasks.view'), async (req, res) => {
  await listTaskGroupRows(req, res, ['device_demo'], 'مهام عروض الأجهزة');
});

router.get('/group/:groupKey', requirePermission('open_tasks.view'), async (req, res) => {
  ...
  await listTaskGroupRows(req, res, config.taskTypes, config.emptyLabel);
});
