...
**قرار موحّد 2026-07-29 — المهنة بين الزبائن والأسماء المقترحة:**
> `clients.occupation` و`candidates.occupation` يقرآن من فئة `occupation` نفسها في `system_lists`، وبنفس resolver وشكل API والعرض والتحقق الخادمي. لا توجد قراءة خاصة بكل كيان، والتحويل من اسم إلى زبون يمرر القيمة نفسها بلا ترجمة. القيمة التاريخية المعطلة تبقى مقروءة مع شارة، ولا يعاد اختيارها كقيمة جديدة.
