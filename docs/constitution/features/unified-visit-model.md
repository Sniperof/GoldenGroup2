...
### `UV-R020` — الزيارة الميدانية الفورية (DEC-011)
مسؤول الفريق يُنشئ زيارة خارج الخطة تبدأ `in_progress` فوراً لزبون في فرعه ضمن مناطق مساره اليوم؛ منع عند الـ cooldown؛ `origin_type='field_initiated'`؛ تبدأ فارغة وتُملأ بالسحب (UV-R019)؛ تُنشئ/تُغلق contact_target. صلاحية `field_visits.create_instant`. (مرجع: DEC-011)
...
### `UV-R021` — مهمة عرض واحدة لكل زيارة
زيارة عرض الجهاز تحتوي `visit_task` واحدة من نوع `device_demo` بالضبط. ويجوز أن تحتوي معها مهاماً من أنواع أخرى، لكن لا يجوز تكرار `device_demo` داخل الزيارة نفسها. الزيارات ذات الأغراض الأخرى لا يلزم أن تحمل مهمة عرض. تعدد محاولات العرض يكون عبر زيارات متعددة تحت قصة `open_task` نفسها.

### `UV-R022` — إنشاء عقد من سياق الزيارة
بعد تسجيل نتيجة `device_demo` ذات عرض مقبول، تستطيع المشرفة المسؤولة بدء إنشاء عقد إذا كانت تملك `contracts.create` وكانت الزيارة ضمن فرعها ومسندة إليها. يُثبت الزبون من الزيارة، ولا يشترط أن يظهر ضمن قائمة زبائنها العامة. اختيار أحد العروض المقبولة داخل العقد اختياري؛ وإذا اختير عرض تطبق قواعد التطابق والتفرّد. هذا التفويض خاص بالعملية ولا ينقل ملكية الزبون.
...
- فعل «إضافة عقد» بعد نتيجة عرض تتضمن عرضاً مقبولاً وعند اجتياز صلاحية العقد وسياسة الزيارة؛ يفتح النموذج بزبون ثابت ويعرض العروض المقبولة غير المباعة كخيارات اختيارية.
