...
### المحور 5.1 — قرار إنشاء مهمة التشغيل من بطاقة الجهاز

> **قرار منتجي معتمد (2026-06-18):** إنشاء مهمة تشغيل الجهاز يتم من سياق **الجهاز نفسه**، لا من سياق الزبون العام. زر الإجراء داخل أجهزة الزبون ينتقل حسب حالة الجهاز:
> `pending_delivery` ⇒ مهمة تسليم، `delivered` ⇒ مهمة تركيب، `installed` ⇒ مهمة تشغيل.

عند ضغط زر **جدولة مهمة تشغيل** على جهاز حالته `installed`، ينشئ النظام مهمة تشغيل لذلك الجهاز بعد استكمال المدخلات التالية:

| المدخل | القاعدة |
|---|---|
| الجهاز المركّب | يُحدَّد تلقائياً من بطاقة الجهاز، ولا يختاره المستخدم يدوياً داخل النموذج |
| موعد التنفيذ | مطلوب كموعد تشغيلي مقترح لتنفيذ الزيارة أو وضعها في خطة الفريق |
| الأولوية | مطلوبة، والافتراضي `medium` إذا لم يغيّرها المستخدم |
| التاريخ المطلوب | مطلوب ويُسجَّل كـ `due_date` لمهمة التشغيل |
| الملاحظات | اختيارية، وتُستخدم لتعليمات التنفيذ أو ظروف الموقع أو تفضيلات الزبون |

يشتق النظام تلقائياً من الجهاز:
- الزبون المرتبط
- الفرع
- العقد إن وجد
- عنوان التركيب النهائي
- `installed_device_id`
- سبب الإنشاء التشغيلي

لا يُطلب من المستخدم إدخال بيانات بيع أو تسليم أو تركيب جديدة عند إنشاء مهمة التشغيل؛ فهذه المهمة تبدأ من جهاز مركّب موجود بالفعل.

### المحور 5.2 — شروط السماح والرفض عند الإنشاء

يسمح النظام بإنشاء `device_activation` فقط عند تحقق الشروط التالية:

| الشرط | الحكم |
|---|---|
| الجهاز محدد ومعروف في `installed_devices` | إلزامي |
| حالة الجهاز الحالية `installed` | إلزامي |
| يوجد عنوان تركيب نهائي قابل للتخطيط | إلزامي |
| لا توجد مهمة تشغيل نشطة لنفس الجهاز | إلزامي |
| التاريخ المطلوب موجود | إلزامي |
| موعد التنفيذ موجود | إلزامي |
| الأولوية موجودة أو يمكن افتراضها | إلزامي |

ويرفض النظام إنشاء مهمة التشغيل في الحالات التالية:
- الجهاز ما زال بانتظار التسليم (`pending_delivery`)
- الجهاز تم تسليمه فقط ولم يُركَّب بعد (`delivered`)
- الجهاز فعّال بالفعل (`active`)
- الجهاز خارج الخدمة أو مسترجع أو في حالة صيانة لا تسمح بالتشغيل
- توجد مهمة تشغيل نشطة لنفس الجهاز
- عنوان التركيب النهائي ناقص أو غير قابل للتخطيط
...
- [x] إنشاء مهمة التشغيل من زر الجهاز يلتزم بمدخلات: جهاز مركّب + موعد تنفيذ + أولوية + تاريخ مطلوب + ملاحظات اختيارية.
- [x] إنشاء مهمة التشغيل مرفوض إذا لم تكن حالة الجهاز `installed` أو كان عنوان التركيب ناقصاً أو وُجدت مهمة تشغيل نشطة لنفس الجهاز.
- [x] منع التكرار مطبّق على مستوى الجهاز لا على مستوى الزبون.
