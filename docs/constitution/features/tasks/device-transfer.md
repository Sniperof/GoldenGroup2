...
`device_transfer` هي مهمة نقل جهاز مفكوك في موقعه بعد مهمة فك ناجحة إلى عنوان مبدئي جديد، من دون تركيب أو تشغيل.
...
- `device_transfer`: نقل جهاز مفكوك بين موقع زبون وموقع زبون، مع إبقاء الحيازة أو نقلها لزبون آخر.
...
- حالة الجهاز `out_of_service`.
- توجد مهمة `device_disconnection` ناجحة سابقة لنفس الجهاز، ونتيجتها واحدة من:
...
- تلغى مهام `periodic_maintenance` القادمة غير المنفذة لنفس الجهاز والزبون القديم، وتلغى روابط `visit_tasks` غير المنتهية التابعة لها، مع تسجيل سبب الإلغاء في `task_activity_log`.
- إذا كانت صيانة دورية لنفس الجهاز والزبون القديم قيد التنفيذ أو انتهت ميدانياً وتنتظر النتيجة، يمنع إتمام النقل حتى تحسم تلك المهمة؛ لا تلغى مهمة ميدانية جارية بصمت.
...
- النقل لا يبدأ إلا بعد الفك الناجح وبقاء الجهاز بحالة `out_of_service` في موقعه، ولا يعني ذلك سحبه إلى الشركة.
