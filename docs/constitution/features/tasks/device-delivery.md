...
### قاعدة `delivery_suspended`

يجوز من تفاصيل الجهاز فقط نقل جهاز عقد الشركة بين `pending_delivery` و`delivery_suspended` بصلاحية مستقلة. عند التعليق تُلغى مهمة التسليم المفتوحة والزيارة المجدولة التي لم يبدأ تنفيذها بسبب مصنّف «تعليق التسليم لغياب الزبون». إذا بدأت الزيارة أو انتهى تنفيذها وبقيت النتيجة قيد الحسم، يُرفض التعليق حتى تُحسم الزيارة.

يبقى الجهاز والرقم التسلسلي محجوزين للعقد، ولا تتغير حالة العقد أو الأقساط أو الذمم أو مهام التحصيل. عند الإعادة إلى `pending_delivery` لا تُنشأ مهمة تلقائياً؛ تعود إمكانية «جدولة مهمة تسليم» للموظف.
...
`serial_number` اختياري في جميع مراحل المسار، بما فيها التسليم الناجح. إذا لم يكن معروفاً تبقى القيمة `NULL` ويجوز استكمالها لاحقاً؛ وإذا أُدخلت قيمة فيجب أن تجتاز قيد التفرد العالمي بعد التطبيع.
