...
| ملف الزبون | `GET /api/app/me` | زبون (بتوكن) | ملف مُقلَّل البيانات من سجل الزبون المرتبط (الاسم الثلاثي/الأرقام وحالة واتساب/عنوان مُحلّ الأسماء/تصنيف/حالة الحساب). الحقول الداخلية لا تُعرَض. |
...
| قوائم العنوان | `GET /api/public/areas` + `/search` | عام | شجرة `geo_units` النشطة فقط؛ قوائم متتالية عبر `parent_id` أو بحث نصي محدود يعيد المسار الكامل. يُغذّي منتقي المحافظة/المنطقة/الناحية/الحي. |
| إنشاء طلب حساب | `POST /api/app/account-requests` | Visitor | `{ form, verificationHandle }` → يحفظ الطلب `Pending` ويستهلك المُعرّف. النموذج يلزم بالاسم الثلاثي وحالة واتساب للرقم الرئيسي، وبحالة واتساب للثانوي عند وجوده. العنوان **بمعرّفات `geo_units` القانونية** (لا نص حر) مع تحقّق المستوى وسلسلة الآباء + لقطة أسماء للمقارنة. **يعيد اللقطة المخزَّنة كاملةً** (§5.4). |
...
**مصدر بيانات شاشة `pending`**: المسار الطبيعي محلي بالكامل — ردّ الإنشاء هو اللقطة كاملةً (الاسم الثلاثي، الرقمان **مطبَّعين** وحالة واتساب لكل منهما، أسماء العنوان الأربعة، العنوان التفصيلي، الملاحظات، `submittedAt`، رقم المرجع) فيُخزَّن كما هو وتُرسم الشاشة بلا نداء ولا تتبّع لأسماء المنتقي. وعند فقدان النسخة المحلية وحدها: تحقّق OTP واحد بغرض `request_status` ثم `…/mine` يعيد **الشكل ذاته حرفياً**، فراسمٌ واحد في التطبيق يكفي للحالتين. ويُعاد فحص `status` عند كل فتح: `active` تعني الموافقة، و`visitor` تعني الرفض أو الأرشفة (تُمسح النسخة المحلية، ورسالة محايدة — لا يُفصح عن السبب).

**الرفض لا يحجب إعادة التقديم**: بعد الرفض يستطيع المستخدم إرسال طلب `account_creation` جديد بحرية (قاعدة «طلب مفتوح واحد» للمعلّق فقط). ويُعرض مصير الطلب المرفوض وسببه **دون حجب** لصاحب الرقم المُثبَت: تلقائياً في ردّ `account/status` عند تمرير `ref` (بلا OTP)، أو عبر `…/mine`. المسار العام يبقى `visitor` صامتاً. القاعدة في `DEC-013 §9.12`.
