...
> **الحالة:** مسودة توصيف — فرع `Authenticate` · **backend الـ auth مكتمل** (م١-٦ + الحذف + صفحة الويب) · **واجهة الأدمن لقسم الطلبات مكتملة** (قائمة + تفاصيل + قرارات) · متبقٍّ: واجهة إدارة الحسابات (إيقاف/تفعيل) + سياسة duplicate_flag
