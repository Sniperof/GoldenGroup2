...
- الإدخال اليدوي المرتبط بشاغر يخضع لنافذة الأهلية نفسها، وتُرفض الطلبات القديمة أو المصطنعة من الخادم بالكود `vacancy_not_applicable`.
...
- `email` — اختياري
- `cvUrl` / `photoUrl` — مرفقات اختيارية في النموذج العام والإدخال اليدوي، ولا يمنع غياب أي منهما إنشاء الطلب
