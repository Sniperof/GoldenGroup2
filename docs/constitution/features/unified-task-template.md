...
### قاعدة موضع تسجيل النتيجة

- نتيجة التنفيذ تخص `visit_task`، لذلك تُسجّل حصراً من صفحة الزيارة المرتبطة.
- صفحة تفاصيل `open_task` تعرض النتيجة وسجل المحاولات فقط، ولا تفتح أي نموذج نتيجة.
- إذا لم تدخل المهمة في الجدولة بعد (`open`, `needs_follow_up`, `assigned`, `in_scheduling`) ولم ترتبط بزيارة نشطة، يجوز إلغاؤها من صفحة التفاصيل.
- الإلغاء المباشر ينقلها إلى `cancelled` في مرحلة الإغلاق، ويلزم سببا من فئة `system_lists` التي تحلها سياسة الإلغاء المشتركة حسب نوع المهمة. يجوز أن تكون فئة الرفض نفسها فقط عندما يكون المعنى التجاري متطابقا؛ وإلا تستخدم فئة إلغاء مستقلة دون تغيير مسار الإلغاء العام.
- من `scheduled` فصاعداً لا يوجد إلغاء مباشر للمهمة؛ يجب توثيق النتيجة أو الرفض من داخل الزيارة.
...
| تسجيل/تعديل نتيجة المهمة من الزيارة | `tasks.results.record` | المسؤول المخول وفق مرحلة الزيارة (D11) |
| الإقفال الإداري للزيارة (`closed`) | `field_visits.edit` | مشرف / مدير |
...
- [ ] side table موجودة (إن وُجدت) مع قيد يمنع التكرار: `visit_task_result_id UNIQUE` للنتيجة الأحادية، أو مفتاح مركب مثل `(visit_task_result_id, subject_id)` عندما تمثل النتيجة الجماعية عدة subjects.
...
- [ ] النوع مسجل في `VISIT_RESULT_TASK_TYPES` وفي سجل نوافذ نتائج صفحة الزيارة.
- [ ] لا يوجد زر أو مسار تسجيل نتيجة من صفحة تفاصيل `open_task`.
- [ ] فئة سبب إلغاء ما قبل الجدولة معرفة صراحة؛ لا يعاد استخدام فئة رفض النتيجة إلا إذا كان المعنى التجاري متطابقا. `gift_delivery` يستخدم فئة إلغاء مستقلة لأن إلغاء محاولة التنفيذ لا يعني رفض المستفيد.
