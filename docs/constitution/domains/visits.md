...
### `V-R020` — الزيارة الميدانية الفورية (DEC-011)
مسؤول الفريق (المشرفة/الفني — D11) يُنشئ زيارة **خارج الخطة** تبدأ فوراً `in_progress` + GPS (D17)، عبر `field_visits.create_instant`. الحُرّاس: الزبون في فرعه **و** حيُّه ضمن مناطق مسار فريقه اليوم؛ **منع** إذا الزبون ضمن `cooldown_until`/`do_not_contact` (DEC-005). `origin_type='field_initiated'`، الفريق يُشتق من `day_schedule` اليوم. تبدأ فارغة وتُملأ بالسحب (V-R019)؛ تُنشئ وتُغلق `contact_target` (`closing_reason='field_initiated_visit'`). الاكتمال موروث من DEC-007. (مرجع: DEC-011 D-FI1..D-FI10)

### `V-R019` — سحب مهام الزبون داخل الزيارة + إلغاء السحب (DEC-010)
الفريق المسؤول عن زيارة `in_progress` يسحب مهام الزبون من **مرحلة قيد الانتظار فقط** (`open`/`needs_follow_up`) عبر قائمة محصورة بـ **فرع الزيارة**، بغض النظر عن موقع المهمة، دون N-Window ولا فلتر أهلية، مرتبة الأقدم أولاً. السحب يحوّل `open_task` إلى `scheduled` وينشئ `visit_task` (`pending`). **إلغاء السحب** متاح فقط للمهمة المسحوبة التي ما زالت `pending` (لا للأصلية ولا بعد تسجيل نتيجة)، فيُحذف الـ `visit_task` وتعود `open_task` إلى `last_waiting_status`. مسار «الإنشاء من داخل الزيارة» مؤجَّل. (مرجع: DEC-010 D-PB1..D-PB8)
...
POST   /field-visits/instant               ← زيارة فورية ميدانية (in_progress الآن) — DEC-011
...
GET    /field-visits/:id/pullable-tasks    ← مهام الزبون القابلة للسحب (open/needs_follow_up، فرع الزيارة) — DEC-010
POST   /field-visits/:id/tasks             ← إضافة/سحب مهمة (cascading، D7 موسّع + DEC-010)
DELETE /field-visits/:id/tasks/:tid        ← إلغاء سحب مهمة pending مسحوبة — DEC-010 D-PB8
