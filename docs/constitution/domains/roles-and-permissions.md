...
### 5.5 إدارة الأدوار ليست صلاحية تشغيلية للفرع

قسم الأدوار والصلاحيات هو سطح أمني مركزي. لذلك:

- السوبر أدمن فقط هو المالك الطبيعي لـ `admin.roles.manage`.
- إسناد الأدوار للمستخدمين مفصول في `admin.roles.users.manage`: السوبر أدمن ومدير الشركة يملكانه بنطاق `GLOBAL`، ومدير الفرع يملكه بنطاق `BRANCH`.
- مدير الشركة يمكن أن يملك قراءة تشغيلية واسعة على كل الفروع، لكن لا يملك تعديل الأدوار افتراضياً.
- مدير الفرع لا يملك `admin.roles.manage` حتى لو كان يدير كل شيء داخل فرعه. يستطيع إسناد دور لمستخدم داخل فرعه عبر `admin.roles.users.manage` بنطاق `BRANCH` فقط.
- المشرفة والفني والتيلماركتر لا يملكون `admin.roles.view` ولا `admin.roles.manage`.
- أي دور يحتاج رؤية إعدادات الأدوار دون تعديل يمنح `admin.roles.view` فقط، وبنطاق `GLOBAL`.

القرار الأمني لا يقول: "هذا المستخدم مدير فرع إذاً مسموح له". القرار يقول:

```text
identity + permission + scope + subject = decision
```

في قسم الأدوار يكون الـ subject هو الدور نفسه أو منح ذلك الدور. وبما أن الأدوار قوالب مركزية في أغلب الحالات، فالنطاق المقبول هنا `GLOBAL` فقط.

#### 5.5.1 مصفوفة إدارة الأدوار المقترحة للأدوار الأساسية

| الدور | `admin.roles.view` | `admin.roles.manage` | `admin.roles.users.manage` | السبب |
|---|---:|---:|---:|---|
| السوبر أدمن | GLOBAL | GLOBAL | GLOBAL | يملك بنية النظام وتوزيع الصلاحيات وإسناد المستخدمين. |
| مدير الشركة | — أو GLOBAL للقراءة فقط عند الحاجة | — | GLOBAL | يرى التشغيل، ويسند الأدوار للمستخدمين، ولا يعدل بنية RBAC افتراضياً. |
| مدير الفرع | — | — | BRANCH | يسند الأدوار لمستخدمي فرعه فقط، ولا يعدل كتالوج الصلاحيات أو منح الأدوار. |
| المشرفة | — | — | — | صلاحياتها تشغيلية داخل النماذج والمهام. |
| الفني | — | — | — | صلاحيات تنفيذ ميداني فقط. |
| التيلماركتر | — | — | — | صلاحيات اتصال وحجز ومتابعة فقط. |

#### 5.5.2 ما الذي يجب فصله لاحقاً

تم فصل حسابات النظام وإسناد الأدوار للمستخدمين إلى `admin.roles.users.manage`. يبقى الفصل الأدق لاحقاً داخل إدارة بنية الأدوار نفسها:

| الصلاحية المستقبلية | الاستخدام |
|---|---|
| `admin.roles.grants.manage` | تعديل صلاحيات ونطاقات الدور. |
| `admin.roles.audit.view` | قراءة سجل تغييرات الأدوار والمنح. |

لا تصبح هذه الصلاحيات نافذة إلا بعد إضافتها رسمياً إلى جدول `permissions` وتحديث الكود والاختبارات وملف `صلاحيات_النظام.xlsx`.
...
| GET | `/api/admin/hr-users` | `admin.roles.view` أو `admin.roles.users.manage` | المستخدمون وحسابات النظام ضمن النطاق |
| POST | `/api/admin/hr-users` | `admin.roles.users.manage` | إنشاء حساب نظام وإسناده لدور |
| PUT | `/api/admin/hr-users/:id` | `admin.roles.users.manage` | تعديل حساب نظام أو تغيير دوره |
