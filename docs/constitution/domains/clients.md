...
2. **الملكية الفردية المشروطة (Personal Single Owner):** يعتبر الموظف مالكاً شخصياً للزبون إذا تم ربطه بـ `client_assignments` وكان الموظف مستوفياً للشروط التالية:
   ...
   - يملك تعيين فرع فعالاً (`user_branch_assignments.status = 'active'`) في فرع الزبون نفسه؛ يمنع الخادم الإسناد العابر للفروع عند الإنشاء والتعديل حتى لو أرسلت الواجهة معرف المستخدم صراحة.
...
4. **ذرّية تعديل الملكية:** تعديل بيانات الزبون واستبدال إسناداته وإلغاء مهامه التسويقية عند الانتقال إلى OP/FOP وكتابة سجل التدقيق عملية واحدة داخل transaction. أي فشل يعيد كل الخطوات ولا يسمح بنجاح جزئي أو بزوال الإسنادات وحدها. القائمة الصريحة الفارغة `assignmentUserIds: []` تعني إزالة جميع الإسنادات، بينما غياب الحقل يعني إبقاءها دون تغيير.

5. **سلامة شبكة الوسطاء:** تعديل الوسيط الأساسي لا يحذف الوسطاء التاريخيين الإضافيين من `referrers`. فشل تحميل الشبكة يجب أن يظهر كخطأ قابل لإعادة المحاولة، ولا يجوز تمثيله للمستخدم كشبكة فارغة.

#### BR-5: ثبات الفرع في التعديل وعملية النقل الصريحة (Branch Immutability & Explicit Transfer)

**القاعدة النافذة (2026-07-22):** فرع الزبون (`branch_id`) يُحدد عند الإنشاء فقط ولا يقبل التغيير عبر نموذج التعديل إطلاقاً. أي طلب `PUT /api/clients/:id` يحمل `branchId` مختلفاً عن الفرع الحالي يُرفض بترميز `400` ورمز `BRANCH_CHANGE_NOT_ALLOWED`، وتعرض الواجهة الفرع في وضع التعديل كحقل قراءة فقط. لا يُسجَّل أي تغيير فرع في `client_audit_log` لأنه لا يحدث.

**خلفية القرار:** الصيغة السابقة لهذا البند («منع النقل عند وجود زيارات نشطة») كانت تضليلاً ثلاثي الطبقات — الواجهة تحذّر، والحارس يفحص، وسجل التدقيق يوثّق نقلاً، بينما جملة `UPDATE` لم تكتب `branch_id` يوماً. أُزيل التضليل واستُبدل بالرفض الصريح.

**العملية المستقبلية المعتمدة مبدئياً (غير مبنية بعد):** النقل بين الفروع سيكون إجراءً مخصصاً (endpoint مستقل بصلاحية خاصة) وفق المبادئ المتفق عليها:

| المحور | القرار |
|---|---|
| حواجز المنع | العمل الجاري فقط: زيارة `in_progress`/`scheduled` أو مهمة `in_progress`؛ العقود النشطة والأقساط لا تمنع النقل |
| القاعدة الناظمة | الماضي يبقى بفرع وقوعه، والمستقبل يتبع الزبون |
| `financial_movements` | لا تُمسّ — دفتر append-only بفرع الوقوع (`occurred_branch_id`) |
| العقود (`branch_id` الإبرام) | لا يُمسّ؛ الخدمة تُدار عبر `service_branch_id` |
| مهام `waiting`/`needs_follow_up` | تُنقل للفرع الجديد بما فيها مهام التحصيل |
| `contact_targets` المفتوحة | تُغلق بسبب مخصص (`branch_transfer`) |
| `client_assignments` الشخصية | تُحذف (المسندون من موظفي الفرع القديم) |
| شرط جغرافي | حي الزبون يجب أن يكون ضمن تغطية الفرع الجديد وإلا رُفض النقل |

**نقطة مؤجلة (Deferred):** خيار «النقل الإداري مع بقاء خدمة الجهاز لدى الفرع القديم» (الوضع ب) مجمَّد؛ تفعيله يتطلب تعديل تعريف الأهلية في التخطيط لأن الشرط `c.branch_id = ot.branch_id` في `planning.ts` و`zoneStudy.ts` يُسقط أي مهمة يختلف فرعها عن فرع زبونها. حتى حسم ذلك، النقل — متى بُني — يكون كاملاً (كل التبعيات المستقبلية تتبع الفرع الجديد).
...
| **TC-11** | محاولة تغيير فرع زبون عبر نموذج التعديل | PUT `/api/clients/:id` | كائن تعديل يحتوي `branchId` مختلفاً عن الفرع الحالي (بغضّ النظر عن حالة الزيارات). | ترميز `400` مع رمز `BRANCH_CHANGE_NOT_ALLOWED` ورسالة أن النقل يتطلب عملية مخصصة. | فرض ثبات الفرع في التعديل (BR-5)؛ النقل حصراً عبر عملية النقل الصريحة عند بنائها. |
