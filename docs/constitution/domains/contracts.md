...
### 🚨 9.1 الثغرة الأولى: غياب أمني مطلق وكامل للتحقق في مسار الديون (Critical Missing Auth & Scopes on Dues Router)
...
- **الأثر التشغيلي:** يمكن لأي شخص أو جهة هجوم خارجي بدون تسجيل دخول استدعاء مسار `GET /api/dues` وقراءة كافة سجلات الديون والمبالغ والأسماء والتواريخ لزبائن الشركة، أو الأخطر تعديل وحذف وشطب الديون والمستحقات وحالات السداد للذمم المالية عبر إرسال طلب تعديل `PUT /api/dues/:id` عشوائي.
- **التوصية:** إدراج بوابات الأمان `requireAuth` وصلاحية `requirePermission('contracts.view_list')` للمسار فوراً وتصفية الديون حسب فرع المستخدم التشغيلي.
