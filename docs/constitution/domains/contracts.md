...
| `source_visit_id` | `INTEGER` | ✅ | — | `FK منطقي → field_visits(id)` | الزيارة الموحدة التي بدأ منها العقد؛ يثبتها الخادم في مسار تفاصيل الزيارة | `2051` |
...
| `source_task_offer_id`| `BIGINT` | ✅ | — | `رابط منطقي → open_task_pre_offers(id)`| معرف العرض المقبول المحدد الذي اختير مصدراً للعقد؛ لا يوجد FK فعلي حالياً | `8014` |
...
#### BR-8: منشأ العقد من الزيارة والمهمة والعرض

1. `source_visit_id` و`source_open_task_id` و`source_task_offer_id` ليست حقلاً واحداً ولا تحمل المعنى نفسه.
2. يظهر بدء الإنشاء من تفاصيل الزيارة بعد نتيجة `device_demo` ذات عرض مقبول واحد على الأقل؛ وعند البدء يثبت الخادم الزيارة والزبون والفرع والمهمة ولا يقبل تبديلها من الحمولة.
3. `source_task_offer_id` اختياري. يجوز حفظ عقد بلا عرض حتى لو احتوت المهمة عروضاً مقبولة.
4. إذا أرسل العرض، يجب أن يكون `accepted`، تابعاً للمهمة والزيارة والزبون والفرع نفسها، وغير مستخدم في عقد حي آخر.
5. لكل عرض مقبول عقد حي واحد كحد أقصى. إلغاء/استبعاد العقد يحرر العرض وفق سياسة التفرّد المعتمدة في GAP-082.
6. قبول العرض وحده لا ينشئ عقداً ولا يطلق مهام التسليم أو التحصيل؛ الأثر التشغيلي يبدأ من العقد الفعلي حسب دورة حياته.
7. العقود اليدوية أو المهاجرة أو المحقونة مباشرة قد تحمل NULL في أي من روابط المصدر. هذه ليست مخالفة تلقائية، ولا يجوز backfill تخميني أو رفضها عند القراءة/التعديل لمجرد نقص روابط تاريخية. الحراس الصارمة تطبق على الكتابات الجديدة بحسب سياقها.
...
| `field_visits` كمصدر | `N:1` اختياري | يحفظ تاريخ العقد | الزيارة التي بدأ منها إنشاء العقد؛ لا تعني أن اختيار عرض كان إلزامياً. |
| عرض مهمة الجهاز كمصدر | `0..1:1` للعقد و`0..1` عقد حي للعرض | اختياري | عند وجوده يخضع لتطابق الحالة والمهمة والزبون والفرع ولحارس منع البيع المزدوج. |
...
- **إنشاء عقد من زيارة مسندة:** لا يضيف نطاق `ASSIGNED` إلى دومين العقود. يبقى المطلوب `contracts.create` بنطاق يسمح بفرع العقد، ثم تتحقق policy سياقية من الزيارة والمسؤول الفعلي عنها. يُجلب زبون الزيارة كسجل مفرد مثبت، لا عبر قائمة الزبائن العامة، ولا ينشأ عن ذلك حق عام على ملف الزبون.
...
| **GET** | `/api/contracts/creation-context/visit/:visitId` | `contracts.create` + policy الزيارة | تجهيز سياق إنشاء عقد من زيارة: الزبون والمهمة المثبتان والعروض المقبولة غير المرتبطة بعقد حي. لا ينشئ عقداً ولا يوسّع صلاحية الزبون. |
| **GET** | `/api/contracts/customer-lookup?q=...` | `contracts.create` + `clients.view_list` | بحث خفيف ومحدود النتائج لاختيار زبون العقد اليدوي؛ يطبق تقاطع نطاقي الفرع والإسناد ولا يحسب إجماليات أو ينزل قائمة الزبائن. |
| **GET** | `/api/contracts/customer-context/:clientId` | `contracts.create` + `clients.view_list` + `clients.view` | يجلب حقول العقد اللازمة للزبون المختار فقط بعد إعادة فحص النطاق نفسه. |
...
| **TC-09** | فتح عقد من زيارة مسندة بعد عرض مقبول | تفاصيل الزيارة → نموذج العقد | مشرفة تملك `contracts.create` والزيارة مسندة إليها | يفتح النموذج بزبون وفرع وزيارة ومهمة مثبتة، دون تحميل قائمة الزبائن العامة | اختبار إيجابي لسياق الزيارة |
| **TC-10** | حفظ عقد من الزيارة دون اختيار عرض | POST `/api/contracts` | سياق زيارة ومهمة صالحان، `sourceTaskOfferId = null` | ينجح الحفظ وتبقى حقول العرض فارغة | العرض اختياري بعد تحقق أهلية فتح المسار |
| **TC-11** | حفظ عقد مرتبط بعرض مقبول | POST `/api/contracts` | عرض مقبول غير مباع يطابق الزيارة والمهمة والزبون والفرع | ينجح الحفظ وتثبت معرفات المصدر ورقم البيعة | المسار المفضّل |
| **TC-12** | محاولة ربط عرض مرفوض أو من مهمة/زبون/فرع آخر | POST `/api/contracts` | `sourceTaskOfferId` غير مطابق | `403` أو `409` دون إنشاء أي سجل تابع | تحقق الخادم deny-by-default |
| **TC-13** | محاولة إنشاء عقد حي ثانٍ للعرض نفسه | POST `/api/contracts` | عرض مستخدم في عقد حي | `409` | حارس منع البيع المزدوج |
| **TC-14** | مشرفة غير مسندة للزيارة | تفاصيل الزيارة أو POST العقد | تملك `contracts.create` لكن ليست مسؤولة عن الزيارة | `403` ولا يكفي ظهور الزبون أو وجوده في الفرع | اختبار Subject سلبي |
| **TC-15** | عقد مهاجر بلا روابط مصدر | GET/PUT عقد تاريخي | `source_visit_id/source_open_task_id/source_task_offer_id = NULL` | يُقرأ ويُعدّل ضمن الصلاحيات المعتادة دون فرض backfill | لا تطبيق رجعي ولا روابط مخترعة |
| **TC-16** | lookup زبون العقد اليدوي | فتح النموذج والبحث | قاعدة زبائن كبيرة | بحث خادمي مؤجل ومحدود النتائج بحقوق النطاق وإسقاط أدنى للحقول؛ لا تنزيل كامل الجدول | اختبار أداء ومنع تسريب |
...
### GAP-089: إنشاء العقد من الزيارة وربط العرض الاختياري ⏳ قيد التنفيذ
- **التضارب:** نموذج العقد يعتمد على قائمة الزبائن العامة؛ لذلك قد تُحجب مشرفة مسندة لزيارة زبون OP/FOP عن العملية، كما أن تحميل القائمة الكاملة يبطئ فتح النموذج. ربط العرض الحالي لا يملك عقد تحقق كاملاً للسياق، ولا تستعمل واجهة الزيارة مساراً ثابتاً للزبون.
- **العقد المعتمد:** بعد نتيجة `device_demo` ذات عرض مقبول يظهر فعل إضافة العقد للمشرفة المسؤولة صاحبة `contracts.create`. يثبت سياق الزيارة والزبون والمهمة؛ ويكون اختيار عرض مقبول غير مباع اختيارياً. لكل عرض عقد حي واحد كحد أقصى.
- **حماية التاريخ:** العقود المهاجرة أو المحقونة مباشرة ذات الروابط الناقصة تبقى صالحة، ولا تخضع لـbackfill تخميني أو رفض رجعي.
- **الأداء:** الإنشاء اليدوي يستخدم lookup زبائن خادمياً مؤجلاً ومحدود النتائج وإسقاطاً أدنى؛ مسار الزيارة يجلب زبوناً مفرداً ولا يستدعي القائمة.
- **النطاق المتوقع:** policy/service مشترك للتحقق، عقد API سياقي، واجهة تفاصيل الزيارة، نموذج العقد، قيود/فهارس لازمة بعد تدقيق DB، واختبارات TC-09..TC-16.
- **المنجز في الخطوة 1 (2026-09-09):** policy مركزية + خدمة سياق + `GET /api/contracts/creation-context/visit/:visitId` للقراءة فقط، مع اختبارات الإسناد والفرع والأهلية والتعدد والعروض المباعة. لم تُوصل الواجهة ولم يتغير `POST /api/contracts` بعد.
- **المنجز في الخطوة 2 (2026-09-09):** رُبط `POST /api/contracts` بالسياق عند إرسال `sourceVisitId`. الخادم يرفض اختلاف الزبون/الفرع/المهمة/صاحب البيعة، ويثبت `source_visit_id`، ويأخذ `source_task_offer_id` ورقم البيعة من العرض المؤهل عند اختياره. عدم اختيار العرض يحفظ الرابطين فارغين.
- **المنجز في الخطوة 3 (2026-09-09):** ثُبّتت روابط المصدر بعد إنشاء المسودة؛ فلا يستطيع `PUT /api/contracts/:id` إضافة/حذف/تبديل الزيارة أو المهمة أو العرض أو رقم البيعة، وتُثبّت هوية الزبون وصاحب البيعة للعقد الناتج عن زيارة. العقود اليدوية/المهاجرة ذات الروابط الناقصة لا يُفترض لها مصدر ولا تُجبر على backfill. أضيفت الهجرة `457_contract_live_source_uniqueness.sql` بقيدين فريدين جزئيين يمنعان أكثر من عقد حي لنفس العرض أو رقم البيعة، مع تحويل تعارض التزامن إلى 409. الواجهة والـlookup السريع والتحقق المرجعي العام عند إرسال عرض بلا `sourceVisitId` ما زالت ضمن العمل اللاحق.
- **المنجز في الخطوة 4 (2026-09-09):** أصبحت تفاصيل الزيارة تستدعي سياق الإنشاء الخادمي وتعرض زر «إنشاء عقد للزبون» داخل مهمة العرض المؤهلة فقط. يفتح الزر `ContractForm` مع `visitId`؛ فيُجلب زبون مفرد بدل قائمة الزبائن، وتُثبّت هوية الزبون والمهمة وصاحب البيعة، وتُعرض العروض المقبولة غير المستخدمة مع خيار صريح للمتابعة دون ربط عرض. كتالوج الجهاز والعنوان والموظفين يُطلب بحسب فرع الزيارة.
- **المنجز في الخطوة 5 (2026-09-10):** أزيل تحميل قائمة الزبائن من فتح نموذج العقد اليدوي. يبدأ البحث بعد حرفين و300ms مع إلغاء الطلب القديم، ويعيد 20 نتيجة خفيفة مع مؤشر وجود المزيد. اختيار الزبون يجلب سياقه المفرد بعد `clients.view`، و`POST /api/contracts` يعيد تطبيق `contracts.create` و`clients.view_list` و`clients.view` مع تقاطع الفرع/الإسناد ويثبت الاسم من قاعدة البيانات. بقي التحقق المرجعي العام عند إرسال عرض بلا سياق زيارة.
- **تصحيح الخطوة 4 (2026-09-10):** إذا احتوت المهمة عرضاً مقبولاً واحداً فقط وكان غير مرتبط بعقد، يطبقه النموذج تلقائياً لتعبئة الموديل والسعر وطريقة الدفع والدفعات/الأقساط، وتبقى القيم قابلة للتعديل. إذا تعددت العروض المقبولة فلا اختيار تلقائي، ويجب على المشرفة تحديد العرض المقصود أو المتابعة صراحةً دون ربط عرض.
...
| **2026-09-09** | `457_contract_live_source_uniqueness.sql` + كود | **GAP-089 / الخطوة 3:** تجميد روابط مصدر العقد بعد الإنشاء، وتثبيت زبون وصاحب بيعة العقد الناتج عن زيارة، وفرض عقد حي واحد لكل عرض مقبول أو رقم بيعة حتى مع الطلبات المتزامنة، دون backfill للعقود التاريخية. |
