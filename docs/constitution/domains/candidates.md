...
- استثناء مقيد للزبون المحجوب: حامل `candidates.link_restricted_lead` يستطيع ربط اسمه المسند بهاتف مطابق تماماً إلى زبون `LEAD` واحد فقط داخل فرع الاسم ومن فروعه الفعالة. يحل الخادم المطابقة دون كشف المعرّف أو التفاصيل مسبقاً، ويضيف مسؤولي الاسم المؤهلين إلى الزبون دون إزالة مسؤوليه الحاليين. أي اختلاف فرع، أو حالة `FOP`/`OP`، أو تعدد مطابقات، أو غياب الإسناد يرفض العملية.
...
| **POST** | `/api/candidates/:id/link-client` مع `restrictedSameBranchLead=true` | `candidates.edit` + `candidates.link_restricted_lead` على موضوع الاسم | يحل تطابق الهاتف المحجوب خادمياً ويربط `LEAD` واحداً داخل الفرع نفسه ثم يضيف مسؤول الاسم. |
