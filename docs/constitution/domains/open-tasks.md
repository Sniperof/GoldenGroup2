...
| `excluded_for_date` | `DATE` | ✅ | — | — | **Legacy:** استبعاد المهمة عن كل فرق الفرع في يوم التخطيط المحدد؛ لا يمثل استبعاد فريق واحد | `"2026-05-25"` |
| `excluded_reason` | `TEXT` | ✅ | — | — | **Legacy:** تبرير استبعاد كل الفرق لليوم المرتبط | `"تم ترحيلها بطلب الإدارة"` |
...
### 2.6 سجل طبقات تنقية التخطيط `planning_task_exclusions`

يمثل قراراً مؤرخاً فوق المهمة ولا يغير `open_tasks.status`:

| الحقل | المعنى |
|---|---|
| `open_task_id`, `branch_id` | موضوع القرار؛ الفرع يطابق فرع المهمة بقيد مركب |
| `planning_date` | يوم التخطيط `D` الصريح |
| `exclusion_scope` | `all_teams` أو `team` |
| `team_key`, `team_snapshot` | إلزاميان لطبقة الفريق ومحظوران لطبقة كل الفرق |
| `reason_code`, `reason_text` | السبب المنظم والتفصيل المقروء |
| `operation_id` | رأس عملية التنقية التي أنشأت القرار |
| `excluded_by`, `created_at` | الفاعل والتوقيت |
| `revoked_at`, `revoked_by`, `revoke_reason` | استعادة غير مدمرة تحفظ التاريخ |

لا يسمح بأكثر من قيد فعّال للطبقة نفسها على `(task, D)`، أو `(task, D, team)` لطبقة الفريق.

### 2.7 رأس عملية التنقية `planning_curation_operations`

يحفظ العملية المفردة أو الجماعية: الفرع، `D`، الفريق، الإجراء، الطبقة، المحدد المطبّع، بصمات الاستعلام والاختيار، السبب، الفاعل، وأعداد الجهات والمهام المتأثرة. هذا السجل هو مرجع التدقيق وليس بديلاً عن سجل الطبقة لكل مهمة.
...
2. **موقع الجهاز (`device`):** للمهام التشغيلية المادية والتركيبية التي ترتبط بالتركيب الجغرافي الفعلي مثل تسليم الجهاز `device_delivery` أو صيانته وتحصيل دفعاته. يتم قراءة عنوان التركيب من `installed_devices.installation_geo_unit_id`، ولا يستخدم العقد كمرجع مكاني.
...
1. **الإلغاء (`cancelled`):** حالة نهائية في مرحلة الإغلاق. الإلغاء المباشر متاح فقط قبل الجدولة، في الحالات `open`, `needs_follow_up`, `assigned`, `in_scheduling`، وبشرط عدم وجود زيارة نشطة بلا نتيجة. السبب إلزامي وتحل سياسة الإلغاء المشتركة فئته حسب نوع المهمة؛ يجوز أن تطابق فئة رفض النتيجة عندما يتطابق المعنى، أو تكون فئة إلغاء مستقلة عندما يعني الإلغاء وقف محاولة التنفيذ لا رفض المستفيد. يحفظ السبب مرجعياً في `cancellation_reason_id` ونصياً في `cancellation_reason`.
...
3. **استبعاد التخطيط:** قيد يومي مستقل، لا حالة مهمة. يطبق على `open`, `needs_follow_up`, `assigned` فقط بطبقة `team` أو `all_teams` في `planning_task_exclusions`.
4. استبعاد `assigned` يحرر الإسناد فقط عند تطابق الفرع و`planning_date` والفريق المقصود، ويعيد `last_waiting_status` أو `open`. لا يمس `in_scheduling` وما بعدها.
5. **عدم التواصل:** قيد على `clients.do_not_contact` وليس استبعاد مهمة. يؤثر في كل فرق وأيام الزبون ولا يلغى من مسار استعادة المهمة.
6. `excluded_for_date` و`excluded_reason` حقلا توافق قديمان يعادلان `all_teams` لليوم المكتوب حصراً؛ لا يستخدمان لتمثيل `team`.
...
- **الاستبعاد الجانبي ليس حالة:** لا توجد قيمة `excluded` ضمن آلة حالات المهمة. هو overlay مؤرخ قابل للاستعادة الدقيقة في مراحل `open`, `needs_follow_up`, `assigned` فقط. قد تعرض الواجهة تسمية مشتقة «مستبعدة» من دون تعديل `status`.

---

## 6. صلاحيات الوصول (Permission Matrix)

| الصلاحية (Permission Key) | النطاق (Allowed Scopes) | الوصف |
|---|---|---|
| `open_tasks.view` | `GLOBAL / BRANCH` | قراءة كشوف المهام وتفاصيلها وسجل الأنشطة — جميع GET endpoints |
| `open_tasks.edit` | `GLOBAL / BRANCH` | إنشاء مهام وتعديل حالاتها وتعيين الفرق ضمن سياسة موضوع المهمة |
| `planning.manage` | `GLOBAL / BRANCH` | قراءة وتنقية حمل فريق يومي بعد التحقق من الفرع و`D` والفريق والمهمة |
| `clients.contact_control.edit` | `GLOBAL / BRANCH` | تفعيل/إلغاء عدم التواصل على فرع تسجيل كل زبون |

**الأدوار الحاملة للصلاحيتين:**

| الدور | `open_tasks.view` | `open_tasks.edit` | النطاق |
|---|---|---|---|
| `SYSTEM_ADMIN` | ✅ | ✅ | GLOBAL |
| `branch_manager` | ✅ | ✅ | BRANCH |
| `telemarketer` | ✅ | ✅ | BRANCH |

> **ملاحظة (GAP-017 ✅):** تم استبدال `marketing_visits.view` / `marketing_visits.update_result` بـ `open_tasks.view` / `open_tasks.edit` في جميع مسارات `openTasks.ts` (migration 174). صلاحيات `marketing_visits.*` لا تزال مستخدمة في `fieldVisits.ts`، `workScopes.ts`، `emergencyResult.ts` وستُعالج عند مراجعة تلك الدومانات.
...
|---|---|---|---|
| `GET /` | `open_tasks.view` | كشف مهام الفرع | `branchId`، `status`، `taskType` |
| `GET /client/:clientId` | `open_tasks.view` | مهام زبون محدد — بدون فلترة فرع للسوبرأدمن | — |
| `GET /device-demo` | `open_tasks.view` | كشف مهام عروض الأجهزة مع بيانات الزيارة | `status`، `visitStatus`، `scheduledDate`، `hideSnoozed`، `hideFutureTasks` |
| `GET /scope/:scopeId` | `open_tasks.view` | مهام نطاق عمل جغرافي محدد | — |
| `GET /:id` | `open_tasks.view` | تفاصيل مهمة واحدة كاملة | — |
| `GET /:id/emergency-result` | `open_tasks.view` | تفاصيل نتيجة الصيانة الطارئة | — |
| `GET /:id/activity` | `open_tasks.view` | سجل أنشطة المهمة مرتب تصاعدياً | — |
| `GET /:id/devices` | `open_tasks.view` | أجهزة المهمة المرفقة | — |
...
| `POST /:id/cancel` | `open_tasks.edit` | إلغاء قبل الجدولة فقط؛ فئة السبب تحلها سياسة الإلغاء المشتركة حسب نوع المهمة | `reasonId`✱ |
...
| `POST /:id/exclude` | `open_tasks.edit` | مسار legacy لاستبعاد `all_teams`؛ لا يستخدم لتنقية فريق واحد | `reason`، `date` الصريح |
| `POST /:id/restore` | `open_tasks.edit` | مسار legacy يستعيد طبقة كل الفرق فقط ويزامن الحقل والسجل المرحّل | `date` الصريح |
| `POST /bulk-exclude` | `open_tasks.edit` | مسار legacy مجمّع لطبقة كل الفرق فقط | `taskIds[]`✱، `reason`، `date` |
| `POST /bulk-restore` | `open_tasks.edit` | مسار legacy مجمّع لاستعادة طبقة كل الفرق فقط | `taskIds[]`✱، `date` |
| `POST /:id/activity` | `open_tasks.edit` | إضافة نشاط / ملاحظة يدوية | `eventType`، `note`، `oldValue`، `newValue` |
| `POST /:id/devices` | `open_tasks.edit` | إضافة جهاز للمهمة | `deviceModelId`، `deviceNameSnapshot`، `quantity` |
...
| **TC-04** | استبعاد مهمة عن فريق واحد في يوم محدد | preview ثم apply للتنقية | مهمة `assigned` تطابق الفرع و`D` والفريق مع السبب. | قيد `team` فعال وتحرير الإسناد المطابق فقط، مع بقاء المهمة في حالة انتظارها السابقة. | لا يجعل حبيبة `(زبون+موقع+D)` قابلة للتقسيم بين فريقين. |
| **TC-05** | استعادة طبقة فريق مع بقاء طبقة أقوى | preview ثم apply للتنقية | مهمة تحمل `team` و`all_teams`. | إلغاء `team` فقط وبقاء المهمة محجوبة بطبقة `all_teams`. | الاستعادة دقيقة ولا تمحو التاريخ. |
...
### ✅ GAP-017: تضارب مسميات الصلاحيات — Legacy Permissions Mismatch (محلول)
* **الموقع:** `routes/openTasks.ts` + `migrations/174_open_tasks_permissions.sql`
* **الحل المُطبَّق:** إنشاء `open_tasks.view` و`open_tasks.edit` في الـ DB، ومنحهما لنفس الأدوار التي كانت تملك `marketing_visits.view` / `marketing_visits.update_result` (SYSTEM_ADMIN-GLOBAL، branch_manager-BRANCH، telemarketer-BRANCH). استبدال جميع الـ 20 استدعاء في `openTasks.ts`. صلاحيات `marketing_visits.*` لا تزال موجودة ومستخدمة في `fieldVisits.ts`، `workScopes.ts`، `emergencyResult.ts` — ستُعالج عند مراجعة تلك الدومانات.
* **التاريخ:** 2026-05-25
...
| **2026-05-25** | `migrations/174_open_tasks_permissions.sql` + `routes/openTasks.ts` | **GAP-017:** استبدال `marketing_visits.view/update_result` بـ `open_tasks.view/edit` — بذر الصلاحيتين ومنحهما للأدوار الثلاثة + استبدال مجمّع لجميع الـ 20 مسار في openTasks.ts. |
