...
|---|---|---|
| `branches.nav` | `GLOBAL / BRANCH` | إظهار إدارة الفروع والأقسام في الدروار فقط، ولا تمنح قراءة أو تعديل بحد ذاتها |
| `branches.lookup` | `GLOBAL / BRANCH / ASSIGNED` | استخدام الفروع كخيارات داخل الحقول والمرشحات بدون فتح سطح إدارة الفروع |
...
| `branches.manage` | `GLOBAL / BRANCH` | إدارة قرارات الفرع الحساسة: الإنشاء بنطاق `GLOBAL` فقط، والحذف وتغيير الحالة (`status`) وتعديل نطاق التغطية (`coveredGeoIds`) على الفرع الموجود ضمن النطاق |
...
**قاعدة النطاق:** إنشاء فرع جديد يحتاج `branches.manage` بنطاق `GLOBAL` أو السوبر أدمن. نطاق `BRANCH` يستخدم لإدارة فرع موجود ضمن `user_branch_assignments`، لأن الإنشاء لا يملك فرع subject سابقا يمكن تقييده به.
...
- **الصلاحية المطلوبة:** `branches.manage` بنطاق `GLOBAL` أو السوبر أدمن.
...
- **الصلاحية المطلوبة:** `branches.edit` أو `branches.manage` على نفس `branchId` (حسب الحقول المرسلة). إذا لم يرسل الطلب `coveredGeoIds`، يجب أن تبقى تغطية الفرع الحالية كما هي.
...
- **الصلاحية المطلوبة:** `branches.manage` على نفس `branchId`.
