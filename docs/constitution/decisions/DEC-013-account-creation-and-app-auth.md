...
12. **جاهزية الإشعارات (خُطّافان فقط، القرار مستقل لاحقاً)**: نحجز نقطة تسجيل رمز دفع الجهاز مرتبطاً بالحساب، ونطلق أحداث نطاق (`account_activated`, `account_suspended`, `account_reactivated`, `account_deleted`) — دون بناء أي منظومة إشعارات (لا توجد خدمة إشعارات في النظام حالياً).
