...
ومن ضمنها `delivery_suspended`: قيمة عرض خام تصف تعليق التسليم الإداري، ولا تمنح تطبيق الزبون أي إجراء كتابي لتغييرها. تبقى إدارة هذه الحالة محصورة بتفاصيل الجهاز الداخلية وصلاحيتها المستقلة.
...
> **مستبدل بتاريخ 2026-08-17:** ألغى `DEC-018` هذا التوجيه واعتمد دومين شكاوى مستقلاً كلياً عن `service_requests`. تبقى بقية بنود DEC-017 نافذة.
...
- `P-DEC017-01`: ✅ أُغلق بواسطة `DEC-018` — الشكوى دومين مستقل، وليست `request_type` على `service_requests`.
