...
| البند | التفصيل |
|---|---|
| **الكيان** | contracts / field_visits / device_demo / clients lookup |
| **الموقع** | `ContractForm` + `POST /api/contracts` + تفاصيل الزيارة + lookup الزبائن |
| **الوصف** | بطء فتح نموذج العقد بسبب تحميل قائمة الزبائن، وعدم قدرة المشرفة المسندة لزيارة زبون خارج قائمة ASSIGNED على بدء العقد، وعدم اكتمال تحقق ربط العرض بالسياق. |
| **الحل المعتمد** | بعد نتيجة عرض ذات عرض مقبول: زر من الزيارة بزبون ومهمة مثبتين؛ اختيار العرض اختياري؛ تحقق خادمي للتطابق؛ عقد حي واحد لكل عرض؛ lookup يدوي خادمي محدود النتائج؛ حماية العقود المهاجرة ذات الروابط الناقصة. |
| **الحالة** | ⏳ قيد التنفيذ — الخطوات 1–5 منجزة: سياق القراءة، حارس `POST`، تثبيت المصدر، قيدا DB، زر الزيارة، وlookup يدوي خفيف مؤجل مع إعادة فحص الزبون عند الاختيار والحفظ. بقي التحقق المرجعي العام لعرض يُرسل بلا سياق زيارة |
| **ملف الدستور** | [domains/contracts.md §3 BR-8](domains/contracts.md) + [features/tasks/device-demo.md §13.3](features/tasks/device-demo.md) + [DEC-002](decisions/DEC-002-contract-ownership-from-task.md) |
