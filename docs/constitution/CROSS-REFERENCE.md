...
### 1.2 `branch_id` (معرف الفرع وعزل الفروع)

يمثل عمود الربط الأساسي لعزل البيانات الجغرافية والتشغيلية بين الفروع بالشركة. جميع الجداول تقريباً تخضع لهذا القيد لمنع تسريب البيانات:

| الجدول | اسم الحقل | NULL? | ON DELETE | قيد ربط فيزيائي؟ | وصف السلوك وتأثير الحذف |
|---|---|---|---|---|---|
| `branches` | `id` | ❌ (PK) | — | — | المصدر الرئيسي للفرع الجغرافي |
| `clients` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع تسجيل الزبون. يمنع حذف الفرع كلياً إذا احتوى على زبائن. |
| `contracts` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع العقد المالي. يمنع حذف الفرع كلياً إذا احتوى على عقود مبرمة. |
| `employees` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع الموظف أو الفني الجاري. يمنع حذف الفرع لوجود موظفين نشطين. |
| `candidates` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع المرشح لقسم التوظيف. يمنع حذف الفرع لوجود مرشحين نشطين. |
| `open_tasks` | `branch_id` | ❌ | `RESTRICT` | ✅ نعم | فرع المهمة المفتوحة الميدانية. يمنع حذف الفرع لوجود مهام. |
| `field_visits` | `branch_id` | ❌ | `RESTRICT` | ✅ نعم | فرع الزيارة الميدانية للفرق. يمنع حذف الفرع لوجود زيارات. |
| `telemarketing_task_lists` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع كشف الاتصالات اليومي. يمنع الحذف لوجود كشوفات. |
| `telemarketing_call_logs` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع سجل مكالمات التسويق. يمنع الحذف لوجود سجلات. |
| `telemarketing_appointments` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع المواعيد المحجوزة هاتفياً. يمنع الحذف لوجود مواعيد. |
| `training_courses` | `branch_id` | ✅ | `RESTRICT` | ✅ نعم | فرع الدورات التدريبية المفتوحة. يمنع الحذف لوجود دورات. |
| `departments` | `branch_id` | ❌ | `CASCADE` | ✅ نعم | أقسام الفرع الإدارية. يُحذف القسم تلقائياً عند مسح الفرع. |
| `user_branch_assignments` | `branch_id` | ❌ | `CASCADE` | ✅ نعم | تعيينات الفروع للمستخدمين. تُمسح التعيينات تلقائياً عند مسح الفرع. |
| `contact_targets` | `branch_id` | ❌ | `RESTRICT` | ✅ نعم | أهداف اتصالات التسويق المحددة للفرع. يمنع حذف الفرع. |
| `roles` | `branch_id` | ✅ | `CASCADE` | ✅ نعم | الأدوار المستنسخة للفرع. تُحذف الأدوار تلقائياً عند مسح الفرع. |
| `referral_sheets` | `branch_id` | ✅ | `SET NULL` | ✅ نعم | فرع كشف الوسطاء المحال. يتم تصفير الفرع عند مسح الفرع الرئيسي. |
